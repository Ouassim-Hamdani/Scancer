...
export const Message = ({data}) => {
  return (
    <div className={data.role==="sender"?"flex-row-reverse flex":"space-x-3 flex flex-row items-end"}>
        ...
        <div className={`${data.role==="sender"?"bg-primary":"bg-white"} px-4 py-2 flex flex-inline   justify-between space-x-3 w-5/12 rounded-xl ${data.role==="sender"?"text-white":"text-gray-800"}`}>
            <p>{data.msg}</p>
